...
class FaceScannerController extends ChangeNotifier {
  ...
  Future<void> _performChecks(Face face) async {
    ...
    switch (_currentChallenge) {
      ...
      case LivenessChallenge.lookStraight:
        _validateLookStraight(face);
        break;
      case LivenessChallenge.turnRight:
        _validateTurnHeadRight(face);
        break;
      case LivenessChallenge.turnLeft:
        _validateTurnHeadLeft(face);
        break;
      case LivenessChallenge.getCloser:
        _validateGetCloser(face);
        break;
  ...
  bool _isFaceValid(Face face) {
    ...
    if (!ovalRect.contains(faceRect.center)) {
      _updateUI(ValidationState.notInPosition);
      return false;
    }
    ...
    if (leftEyeOpen < 0.5 || rightEyeOpen < 0.5) {
      _updateUI(ValidationState.eyesClosed);
      return false;
    }
    return true;
  }

  void _validateLookStraight(Face face) {
    _updateUI(ValidationState.notLookingStraight);
  ...
  void _validateTurnHeadRight(Face face) {
    final y = face.headEulerAngleY ?? 0.0;
    _updateUI(ValidationState.turnRight);
  ...
  void _validateTurnHeadLeft(Face face) {
    final y = face.headEulerAngleY ?? 0.0;

    _updateUI(ValidationState.turnLeft);
    if (y > 15) {
      Future.delayed(const Duration(milliseconds: 500), () {
  ...
  void _validateGetCloser(Face face) {
    final Size screenSize = MediaQuery.of(navigatorKey.currentContext!).size;
    final Size imageSize = Size(
      _cameraController.cameraController.value.previewSize!.height,
      _cameraController.cameraController.value.previewSize!.width,
    ...
    _updateUI(ValidationState.getCloser);
